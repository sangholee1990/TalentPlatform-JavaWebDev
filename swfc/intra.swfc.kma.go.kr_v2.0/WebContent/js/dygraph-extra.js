...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    for (i = 0; i < labels.length; i++) {
        ...
        if (labelVisibility[i]) {
            ...
            //     of this file (UTF-8).
        	if(labels[i].indexOf("<sub>") > 0 || labels[i].indexOf("<sup>") > 0){
        		var si = labels[i].indexOf("<sub>") > 0 ? labels[i].indexOf("<sub>") : labels[i].indexOf("<sup>");
        		var ei = labels[i].indexOf("</sub>") > 0 ? labels[i].indexOf("</sub>") : labels[i].indexOf("</sup>");
        		var tops = labels[i].indexOf("<sub>") > 0 ? 3 : -3;
        		//앞 
        		txt = "- " + labels[i].substring(0, si);
        		ctx.fillStyle = colors[usedColorCount];
        		ctx.fillText(txt, labelsX, labelsY);
        		labelsX = labelsX + ctx.measureText(txt).width;
        		//중간
        		txt =  labels[i].substring(si + 5, ei);
        		ctx.font = 'bold 10px serif';
        		ctx.fillText(txt, labelsX, labelsY + tops);
        		labelsX = labelsX + ctx.measureText(txt).width;
        		//나머지
        		ctx.font = options.legendFont;
        		txt =  labels[i].substring(ei + 6, labels[i].length);
        		ctx.fillText(txt, labelsX, labelsY);
