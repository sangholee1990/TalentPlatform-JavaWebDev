    ...
    Echarts.prototype = {
        ...
        addData: function (seriesIdx, data, isHead, dataGrow, additionData) {
            ...
            for (var i = 0, l = params.length; i < l; i++) {
                ...
                var seriesItem = optionRestore.series.filter(function(a,b){
                	if(a.name == optionRestore.legend.data[seriesIdx]) return true;
                	return false;
                })[0];
                ...
                if (seriesItem) {
                    ...
                    var seriesXData = optionRestore.xAxis[0].data;
                    var addDataIndex = $.inArray(additionData, seriesXData);
                    if(addDataIndex > -1){
                    	seriesItemData[addDataIndex] = data;
                    	mSeriesItemData[addDataIndex] = data;
                    }else{
                    ...
                    if (additionData != null) {
                        ...
                        if (seriesItem.type === ecConfig.CHART_TYPE_PIE && (legend = optionRestore.legend) && (legendData = legend.data)) {
                            ...
                            if (optionRestore.xAxis[axisIdx].type == null || optionRestore.xAxis[axisIdx].type === 'category') {
                                ...
                                axisDataIndex = $.inArray(additionData, axisData);
			                    if(axisDataIndex > -1){
			                    	axisData[axisDataIndex] = additionData;
			                    	mAxisData[axisDataIndex] = additionData;
			                    }else{
            ...
            if (seriesArray.length > 0) {
                ...
                if (this._lastDataIndex != dataIndex || this._lastSeriesIndex != seriesIndex[0]) {
                    ...
                    if (typeof formatter === 'function') {
                        ...
                        this._curTicket = NaN;
                        formatter = '<span>' + this._encodeHTML(categoryAxis.getNameByIndex(dataIndex)) + '</span>';
                        for (var i = 0, l = seriesArray.length; i < l; i++) {
                        	var colorIdx = $.inArray(seriesArray[i].name, this.myChart._optionRestore.legend.data)
                            formatter += '<span class="date" data-idx="' + colorIdx + '" style="color:' + this.zr.getColor(colorIdx) + '">' + this._encodeHTML(seriesArray[i].name || '') + ' : ';
                            data = seriesArray[i].data[dataIndex];
                            data = this.getDataFromOption(data, '-');
                            formatter += (data instanceof Array ? data : this.numAddCommas(data)) + '</span>';
                        }
                        this._tDom.innerHTML = formatter;
                        var format = $(this._tDom).find('.date').sort(function(a, b){
                        	var aName = parseInt($(a).data('idx'));
                        	var bName = parseInt($(b).data('idx')); 
                        	return ((aName < bName) ? -1 : ((aName > bName) ? 1 : 0));
                        });
                        $(this._tDom).append(format);
                    }
                }
