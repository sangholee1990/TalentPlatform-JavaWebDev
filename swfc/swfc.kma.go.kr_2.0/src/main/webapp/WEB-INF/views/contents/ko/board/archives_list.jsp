...
<div id="wrap_sub">
    ...
    <div id="data">
		...
		<form name="searchForm" id="searchForm" action="archives_list.do"  method="post">
	        <input type="hidden" name="iPage" id="iPage" value="${pageNavigation.nowPage}" />
			<div class='searchbox'>
				<select class='sel_sort' name="board_section_cd" style="margin-right: 5px;">
					<option value="" <c:if test="${param.board_section_cd eq ''}">selected="selected"</c:if>>[전체]</option>
					<c:forEach var="o" items="${boardSubCodeList}" varStatus="status">            
		            	<option value="${o.code}" <c:if test="${param.board_section_cd == o.code }">selected="selected"</c:if>>${o.code_ko_nm}</option>
            		</c:forEach>
				</select>
				<select class='sel_sort' name="search_target" style="margin-right: 5px;">
		...
		<table summary='자료실' class="list">
			<caption class='txt_hidden'>자료실</caption>
			<colgroup>
				<col width='10%'/>
				<col width='15%'/>
				<col width='45%'/>
			...
			<c:forEach var="item" items="${list}" varStatus="status">
	               ...
	               <c:if test="${empty list}"> 
					<tr>
						<td colspan="5">컨텐츠가 존재하지 않습니다.</td>
					</tr> 
				</c:if>
			</tbody>
		</table><!-- //bbs -->
