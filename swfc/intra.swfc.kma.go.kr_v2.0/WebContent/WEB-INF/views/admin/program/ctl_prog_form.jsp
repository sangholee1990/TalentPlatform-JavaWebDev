...
<div class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10">
			...
			<!-- content area start -->
	        <form name="frm" id="frm" class="form-horizontal" role="form" action="ctl_prog_submit.do" method="post">
	        	<input type="hidden" name="clt_prog_seq_n" value="${info.CLT_PROG_SEQ_N}" >
	        	<c:if test="${info.CLT_PROG_SEQ_N != '' && info.CLT_PROG_SEQ_N ne null}">
	        	<div class="form-group form-group-sm">
			    	<label for="clt_prog_seq_n" class="col-sm-2 control-label">자료코드 번호</label>
			    	<div class="col-sm-5">
			      		<input type="text" class="form-control" placeholder="자료코드" value="${info.CLT_PROG_SEQ_N}" disabled="disabled">
			    	</div>
			  	</div>
			  	</c:if>
			 	<div class="form-group form-group-sm">
			 	...
			 	<div class="form-group form-group-sm">
			  	...
			  	<div class="form-group form-group-sm">
				    <div class="col-sm-7 text-right">
				    	...
				    	<c:if test="${info.CLT_PROG_SEQ_N eq null }">
...
function insertSmsUser(){
	...
	if(confirm('등록하시겠습니까?')){
		$('#frm').attr("action", "ctl_prog_submit.do");
		frm.submit();
	}
}
...
function modifySmsUser(){
	...
	if(confirm('수정하시겠습니까?')){
		$('#frm').attr("action", "ctl_prog_modify.do");
		frm.submit();
	}
}
...
function deleteSmsUser(){
	if(confirm('삭제하시겠습니까?')){
		$('#frm').attr("action", "ctl_prog_delete.do");
		frm.submit();
	}
}
</script>	
