...
<!--[if lt IE 9]><script type="text/javascript" src="<c:url value="/resources/common/js/flashcanvas.js"/>"></script><![endif]-->
<script type="text/javascript" src="<c:url value="/resources/common/js/dygraph-combined.js"/>"></script>
<script type="text/javascript">
Dygraph.prototype.loadChartData = function(type, sd, ed) {
	var chart = this;
	var dummy = new Date().getMilliseconds();
	$.ajax({
		url : "<c:url value="/ko/current/query.do"/>?dummy=" + dummy,
		cache:false,
		data : {
			type : type,
			sd : sd,
		...
		$.each(data.data, function(key, val) {
			...
			//val[0] = new Date(val[0].substring(0, 4), val[0].substring(4, 6)-1, val[0].substring(6, 8), val[0].substring(8, 10), 00, 00);
		});
		chart.updateOptions({
			file:data.data
		});
...
var chartGraphManager = new function() {
	...
	this.autoRefresh = function() {
		...
		/*
		date = new Date();
		date.setHours(date.getHours() - 9); //현재시간을 가져와 UTC 시간으로 맞춘다.
		var ed =  date.format('yyyymmddhh', true);
		date.setDate(date.getDate() -1);
		var sd = date.format('yyyymmddhh', true);
...
// 		param.height = $(box).height();
	...
	this.addProtonFlux = function(param) {
		this.add('PROTON_FLUX',$.extend({}, {
			...
			ylabel: 'pfu(cm<sup>-2</sup>s<sup>-1</sup>sr<sup>-1</sup>)',
			yAxisLabelWidth:60,
			strokeWidth:2,
			labelsKMB: true,			
			highlightCircleSize:4,
