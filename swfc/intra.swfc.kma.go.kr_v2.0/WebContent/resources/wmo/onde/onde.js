...
onde.PRIMITIVE_TYPES = ['string', 'number', 'integer', 'boolean', 'array', 'object','addAll'];
...
onde.Onde = function (formElement, schema, documentInst, opts) {
    ...
    console.log(onde.options.view );
    
    // Object property adder
//    this.panelElement.find('.property-add').live('click', function (evt) {
//    this.panelElement.find('.property-add').on('click', function (evt) {
    	$('.property-add').click(function(evt){
        evt.preventDefault();
...
//    this.panelElement.find('.item-add').on('click', function (evt) {
    	$('.item-add').click(function(evt){
        evt.preventDefault();
        _inst.onAddListItem($(this));
    });
    // Collapsible field (object and array)
//    this.panelElement.find('.collapser').live('click', function (evt) {
//    this.panelElement.find('.collapser').on('click', function (evt) {
    $('.collapser').click(function(evt){
    	alert(1);
        var collapser = $(this);
...
//    this.panelElement.find('.field-delete').on('click', function (evt) {
   	$('.field-delete').click(function(evt){
        evt.preventDefault();
...
//    this.panelElement.find('.field-type-select').on('change', function (evt) {
    $('.field-type-select').change(function(evt){
        evt.preventDefault();
...
onde.Onde.prototype.eventTrigger = function(){
...
onde.Onde.prototype.renderObject = function (schema, parentNode, namespace, data) {
    ...
    /*
     * 2014.08.29 
     * For json schema reference 
     * 
     */
    if ( props.$ref != "" && props.$ref != undefined ) {
    	props = this.documentSchema.definitions[props.$ref] || {};
    }
    ...
    // Toolbar if needed
    if (showEditBar && onde.options.view == false ) {
    	
    	console.log(fieldValueId + "------------ showEditBar ");
...
onde.Onde.prototype.renderEnumField = function (fieldName, fieldInfo, valueData) {
   ...
   // console.log(typeof fieldInfo['default'] + " " +  fieldInfo.type + " " + fieldInfo.enum.indexOf(fieldInfo['default'])  );
...
//    		&& fieldInfo.enum.indexOf(fieldInfo['default']) >= 0) {
    ...
    if (fieldInfo && fieldInfo.enum) {
    	...
    	if ( fieldInfo.enum.$ref != undefined ){
       		fieldInfo.enum = this.documentSchema.codeList[fieldInfo.enum.$ref];
       	}
        ...
        if (fieldInfo.enum.length > 1) {
            ...
            if (!fieldInfo.required) {
                ...
                fieldValueNode.append('<option value="">no selected</option>');
            }
            for (var iev = 0; iev < fieldInfo.enum.length; iev++) {
                var optN = $('<option></option>');
                $.each(fieldInfo.enum[iev], function(name, value) {
                    //console.log(name + '=' + value);
                    optN.text(name);
                    
                    // xml value 처리는 name기준으로 함.
                    // <MD_MaintenanceFrequencyCode codeListValue="asNeeded" 
                    //  	codeList="./resources/codeList.xml#MD_MaintenanceFrequencyCode"/>
                    optN.attr("value",value);
                    //optN.attr("value",name);
                    // Select the value
                    if (hasSelected && selectedValue == value) {
    ...
    if ( onde.options.view == true ){
    	if ( valueData == null ) valueData = "";
    	fieldValueNode = $('<span></span>').text(valueData);
    }
...
onde.Onde.prototype.renderFieldValue = function (fieldName, fieldInfo, parentNode, valueData) {
    ...
    /**
     * 2014.08.29
     * For json schema reference $ref
     */
     
    if ( fieldInfo.items && fieldInfo.items.$ref != "" && fieldInfo.items.$ref != undefined ) {
    	fieldInfo.items = this.documentSchema.definitions[fieldInfo.items.$ref] || {};
    }
    
    if ( fieldInfo.$ref != "" && fieldInfo.$ref != undefined ) {
    	fieldInfo = this.documentSchema.definitions[fieldInfo.$ref] || {};
    }
    ...
    if (!fieldInfo || !fieldInfo.type || fieldInfo.type == 'any') {
        ...
        if ( onde.options.view == true && !fieldInfo.enum ){
        	valueContainer.text(valueData);
        }else{
        ...
        if (valueData) {
            for (var idat = 0; idat < valueData.length; idat++) {
                ...
                if ( valueData[idat]["@type"] ){
	                for (var tmpItem in fieldInfo.items){
	            		if ( valueData[idat]["@type"] == fieldInfo.items[tmpItem].name ){
	            			//itemTypes = [tmpItem.items];
	                        itemSchema = fieldInfo.items[tmpItem];
	            		}
	            	}
                }
...
onde.Onde.prototype.renderObjectPropertyField = function (namespace, baseId, fieldInfo, propName, valueData) {
     ...
     * 
     */
    if ( onde.options.seqHidden == true && fieldInfo.display == false ){
    	$(rowN).css("display", "none");
    }
    ...
    if (fieldInfo) {
        ...
        if (typeof fieldInfo == 'string') {
             ...
             * For json schema reference $ref
             */
            } else if (typeof fieldInfo.$ref != '') {
                fieldInfo = this.documentSchema.definitions[fieldInfo.$ref ];
                return this.renderObjectPropertyField(namespace, baseId, fieldInfo, propName, valueData);
    ...
    if ((fieldType == 'object' && fieldInfo.display != 'inline') || fieldType == 'array') {
        ...
        if ( onde.options.view == true && fieldType == 'array' && ( valueData == null || valueData.length < 1 ) ){
        	//$(valN).append( $('<span style="margin-left : 30px;"></span>').text("has no data"));
        	$(valN).append( $('<small></small>').
            addClass('description').
            append($('<em></em>').text("NO DATA"))
            );
        }
...
onde.Onde.prototype.renderListItemField = function (namespace, fieldInfo, index, valueData) {
    ...
    if (fieldInfo.type == 'object' && fieldInfo.display == 'inline') {
        ...
        if (collectionType) {
            ...
            labelN.append($('<small></small>').append(' ').append($('<button></button>').
        		...
        		labelN.append($('<small></small>').append(' '));
        	}
        	deleterShown = true;
        }
    }
    ...
    if (!deleterShown) {
        ...
        valN.append($('<small></small>').append(' ').append($('<button></button>').
    		...
    		valN.append($('<small></small>').append(' '));
    	}
    }
    return rowN;
};
...
onde.Onde.prototype._buildProperty = function (propName, propInfo, path, formData) {
    ...
    if (dataType == 'object') {
        ...
        if (propInfo.required && !result.data.length) {
            ...
            console.log('array value-required = ' + path);
        }
    } else {
        var valData = null;
        if (formData) {
        ...
        if (dataType == 'boolean') {
            ...
            if ( propInfo.minimum ){
            	result.minimum = propInfo.minimum;
            }
            
            if ( propInfo.maximum ){
            	result.maximum = propInfo.maximum;
            }
            ...
            if (dataType == 'integer') {
                ...
                if ( result.minimum != null && result.minimum > result.data ){
                    result.errorCount += 1;
                    result.errorData = 'minimum-error';
                }
                
                if ( result.maximum  != null && result.maximum < result.data   ){
                    result.errorCount += 1;
                    result.errorData = 'maximum-error';
                }

            } else if (dataType == 'string') {
            	if ( propInfo.maxlength == undefined && propInfo.format == undefined ) {
            		result.data = valData;
            	}else{
            		
            		if( propInfo.maxlength != undefined  && this._fieldLength(valData, propInfo.maxlength) == false) {
            			console.warn("maxlength-error: " + dataType + " (" + fieldName + ")");
            			result.errorCount += 1;
            			result.errorData = 'maxlength-error';
            			result.maxlength = propInfo.maxlength;
            		}else if ( propInfo.format == "date-time" && this._isValidDatefunction(valData) == false ){
        				console.warn("datetime-error: " + dataType + " (" + fieldName + ")");
            			result.errorCount += 1;
            			result.errorData = 'datetime-error';
            			result.maxlength = propInfo.maxlength;
        			}else{
        				result.data = valData;
        			}
            ...
            if (propInfo && propInfo.required) {
                if (typeof propInfo['default'] != 'undefined') {
                    ...
                    result.errorCount += 1;
                    result.errorData = 'value-required';
                    console.log('field value-required = ' + this._fieldNameToID( "fieldvalue-" + path + "-" + propName ) );
                }
            }
            if (!propInfo) {
    ...
    if (result.errorCount > 0) {
        ...
        this._buildErrorMessage(dataType, fieldId, propName, result)
    }
...
onde.Onde.prototype._isValidDatefunction = function (datestring) {
	var stamp = datestring.split("T");
    var validDate = !/Invalid|NaN/.test(new Date(stamp[0]).toString());
    stamp[1].replace("Z","");
    var validTime = /^(([0-1]?[0-9])|([2][0-3])):([0-5]?[0-9])(:([0-5]?[0-9]))(:([0-5]?[0-9]))?$/i.test(stamp[1]);
    return (validDate && validTime);
}

onde.Onde.prototype._IsURL =  function(url) {
...
onde.Onde.prototype._fieldLength = function(str, param) {
	var resultSize = 0;
	 if(str == null){
	  return 0;
	 }
	 ...
	 return  (resultSize <= param);
}


onde.Onde.prototype._buildErrorMessage = function (dataType, fieldId, propName, error){

	if ( dataType != "object" && dataType !=  "array" ){
	
		var errMsg = error.errorData;
		
		if ( error.errorData == "value-required" ){
			errMsg = propName + "는 필수 입력 값입니다. ";
		}else if ( error.errorData == "value-error" ){
			
			if ( dataType == "number" ){
				errMsg = propName + "는 숫자만 입력 가능합니다.";
			}else if ( dataType == "integer" ){
				errMsg = propName + "는 정수형 숫자만 입력 가능합니다.";
			}
		}else if ( error.errorData == "maxlength-error"){
				errMsg = propName + "는 최대 " + error.maxlength + "자 까지 입력 가능합니다.";
		}else if ( error.errorData == "maximum-error" ) {
			errMsg = propName + "는 최대 " + error.maximum + " 까지 입력 가능합니다.";
		}else if ( error.errorData == "minimum-error" ) {
			errMsg = propName + "는 최소 " + error.minimum + " 까지 입력 가능합니다.";
		}else if ( error.errorData == "datetime-error" ) {
			errMsg = "날짜  입력 양식을 다시 체크하세요. 날짜 입력 양식은 YYYY-MM-DD HH:MM:SS 입니다. ";
		}
		
		$('#' + fieldId).append('<div class="alert-box error"><span>error: </span>' + errMsg + '</div>');
    }
...
onde.Onde.prototype._buildObject = function (schema, path, formData) {
    ...
    // any of one type 
    // object array 의 abstract class의 extends class 명 설정
    // EX_GeographicExtent.java 참조
    if ( schema.properties == undefined && schema.length > 1 ){
    	console.log( schema.length + " " + formData[path + ".@type"] + " == " + schema.properties )

    	for (var tmp in schema ){
    		tmpobj = schema[tmp];
    		console.log("2---" + tmpobj["name"]);
    		if ( formData[path + ".@type"] == tmpobj["name"]){
    			props = tmpobj.properties;
    			continue;
    		}
    	}
    ...
    if (!schema || schema.additionalProperties || !schema.properties) {
        ...
        for (var fieldName in formData) {
            ...
            if (fieldName.startsWith(cpath)) {
                ...
                console.log(propName + " result.data lenght = " + result.data.length);
                ...
                if (dataType === 'object') {
                    ...
                    if ( propName == "@type" ) dataType = "string";
...
onde.Onde.prototype.getData = function (opts) {
    ...
    this.formElement.find('.alert-box').remove();
