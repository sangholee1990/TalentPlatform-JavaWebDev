...
public class HDF5Converter extends JInternalFrame {
	...
	/** Save data as ascii text. */
	public void saveToAscii(File outFile) throws Exception {	
        ...
        try{
        	...
        	for (int i=0; i<rows; i++){
        		...
        		outt.write(String.valueOf(table.getValueAt(i, 0))+",");
	        	
        		for (int j=1; j<cols; j++){
					outt.write(String.valueOf(table.getValueAt(i, j))+",");
	    		}
				outt.newLine();
	    	}
			outt.close();
        ...
        try{
	       ...
	       // int count = 0;
	        ...
	        for (int i=0; i<rows; i++){
	        	...
	        	//x = value;
    			//first = x / value; //16筌욊쑴�붹에占쏙옙占쏙옙癒�쁽�깍옙
    			//last = x - (first * value); //16筌욊쑴�붹에占쏙옙占쏙옙癒�쁽�깍옙
    			//fos.write(first);	
	...
	/** data as text. */
	public JTable getData() throws Exception {	
		return table;
	}
