...
<div id="contents" class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10">
			...
			<c:choose>
				<c:when test="${notice.board_seq == null}">공지사항 등록</c:when>
				<c:otherwise>공지사항 수정</c:otherwise>
			</c:choose>
			</h4>
			<!-- content area start -->
			
			<form:form commandName="notice" role="form" action="notice_submit.do" method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<div class="form-group form-group-sm">
			    	...
			    	<div class="col-sm-3">
			    		...
			    		<c:forEach var="o" items="${subList}" varStatus="status">            
			            	<option value="${o.code}" <c:if test="${notice.site_code_cd == o.code }">selected="selected"</c:if>>${o.code_ko_nm}</option>
	            		</c:forEach>
			           </form:select>
			    	</div>
			  	</div>
			   ...
			   <div class="form-group form-group-sm">
			  	...
			  	<div class="form-group form-group-sm">
			  		<label for="popup_yn" class="col-sm-2 control-label">팝업여부</label>
			  		<div class="col-sm-3">
			  			<form:select path="popup_yn" class="form-control">
			  				<form:option value="Y" label="표출"/>
			  				<form:option value="N" label="미표출"/>
			  			</form:select>
			  		</div>
				...
				<div class="form-group">
		  		...
		  		<c:choose>
					<c:when test="${notice.board_seq == null}">
						<div class="form-group form-group-sm">
...
$(function() {
	...
	$(paramForm).find('input[type=hidden]').each(function() {
		$("#notice").addHidden($(this).attr("id"), $(this).val());
	});
...
function removeFile(id, link) {
	...
	if(linkId != null || typeof linkId != 'undefined'){
		$("#notice").append("<input type=\"hidden\" name=\"delete_file_seq\" value=\""+linkId+"\"/>");
	}
	$("#" + id).parent().remove();
    $(link).parent().remove();
