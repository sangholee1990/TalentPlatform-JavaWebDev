...
$(function() {
	...
	var chartOption = {axisLabelColor:'white'};
	  <c:choose>
		...
		<c:when test="${param.type == 'ELECTRON_FLUX_SWAA'}">
			<c:out value="chartGraphManager.addElectronFluxSWAA(chartOption);"/>
		</c:when>
		<c:when test="${param.type == 'ELECTRON_FLUX_SWAA_2'}">
			<c:out value="chartGraphManager.addElectronFluxSWAA2(chartOption);"/>
		</c:when>
		<c:when test="${param.type == 'GOES_MAG_SWAA'}">
			<c:out value="chartGraphManager.addGoesMagSWAA(chartOption);"/>
		</c:when>
