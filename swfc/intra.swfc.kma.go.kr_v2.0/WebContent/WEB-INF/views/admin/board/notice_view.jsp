...
<div id="contents" class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10">
			<h4 class="page-header">공지사항 보기</h4>
        <table class="table table-condensed">
            <tr>
                <th class="active text-center" width="90">사이트 구분</th>
                <td><spring:escapeBody>${notice.site_nm}</spring:escapeBody></td>                  
            </tr>
            <tr>
                <th class="active text-center">표출여부</th>
                <td><spring:escapeBody>${notice.use_yn eq 'Y' ? '표출' : '미표출'}</spring:escapeBody></td>                  
            </tr>
            <tr>
                <th class="active text-center">팝업여부</th>
                <td><spring:escapeBody>${notice.popup_yn eq 'Y' ? '표출' : '미표출'}</spring:escapeBody></td>                  
            </tr>
            <tr>
                <th class="active text-center">등록일</th>
                <td><spring:escapeBody>${ notice.create_date}</spring:escapeBody></td>                  
            </tr>
            <tr>
                <th class="active text-center">제목</th>
                <td><spring:escapeBody>${notice.title}</spring:escapeBody></td>                  
            </tr>
            	...
            	<th class="active text-center" style="vertical-align: middle;">첨부파일</th>
              	<td>
              		<c:forEach var="fileList" items="${noticeFileList}">
              			<a href="archives_download.do?board_file_seq=${fileList.board_file_seq}"><spring:escapeBody>${fileList.filename}</spring:escapeBody></a><br/>
              	  	</c:forEach>
              </td> 
		    </tr>
        </table>
...
$(function() {	
	$("#contents :button").click(function() {
		...
		case "수정":
			...
			$(paramForm).addHidden("board_seq", ${notice.board_seq});
			$(paramForm).attr("action","notice_form.do");
			$(paramForm).submit();
			break;
		case "삭제":
			if(confirm("삭제하시겠습니까?")) {
				...
				$(paramForm).addHidden("board_seq", ${notice.board_seq});
				$(paramForm).attr("action","notice_del.do");
				$(paramForm).submit();
			}
			break;
		case "목록":
