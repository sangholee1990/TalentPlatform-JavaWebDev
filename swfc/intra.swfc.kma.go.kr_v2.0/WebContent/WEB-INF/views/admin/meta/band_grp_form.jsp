<%@ page language="java" contentType="text/html;charset=utf-8" pageEncoding="UTF-8" trimDirectiveWhitespaces="true"
	import="java.util.*,com.gaia3d.web.dto.BandGroupDTO,org.springframework.util.*"
...
<div id="contents" class="container">
	...
	<form:form commandName="bandGroupDTO" action="band_grp_submit.do" method="post" class="form-horizontal" role="form">
	<input type="hidden" name="clt_tar_band_grp_seq_n" value="${bandGroupDTO.clt_tar_band_grp_seq_n}"/>
	...
	<div class="form-group form-group-sm">
    	<label for="clt_tar_band_grp_cd" class="col-sm-2 control-label">코드</label>
    	<div class="col-sm-5">
      		<form:input path="clt_tar_band_grp_cd" class="form-control"/>
    	</div>
  	</div>
  	
	<div class="form-group form-group-sm">
    	<label for="clt_tar_band_grp_kor_nm" class="col-sm-2 control-label">한글명</label>
    	<div class="col-sm-5">
      		<form:input path="clt_tar_band_grp_kor_nm" class="form-control"/>
    	</div>
  	</div>


	<div class="form-group form-group-sm">
    	<label for="clt_tar_band_grp_eng_nm" class="col-sm-2 control-label">영문명</label>
    	<div class="col-sm-5">
      		<form:input path="clt_tar_band_grp_eng_nm" class="form-control"/>
    	</div>
  	</div>
	...
	<div class="form-group">
    	<label for="clt_tar_band_grp_desc" class="col-sm-2 control-label">설명</label>
    	<div class="col-sm-5">
			     <form:textarea path="clt_tar_band_grp_desc" class="form-control" rows="5"/>
		</div>
  	</div>
...
$(function() {
	...
	$(paramForm).find('input[type=hidden]').each(function() {
		$("#bandGroupDTO").addHidden($(this).attr("id"), $(this).val());
	});
	...
	$("#contents :button").click(function() {
		...
		case "cancelBtn":
			...
			//등록
			<c:if test="${mode eq 'new' }">
				$(paramForm).attr("action", "band_grp_list.do");
			</c:if>
	...
	$("#bandGroupDTO").validate({
		...
		rules: {
			...
			clt_tar_band_grp_eng_nm: {
				alphanumeric : true,
				fieldLength : 50
			},
			clt_tar_band_grp_desc: {
				fieldLength : 2000 
			}
		},
		messages: {
