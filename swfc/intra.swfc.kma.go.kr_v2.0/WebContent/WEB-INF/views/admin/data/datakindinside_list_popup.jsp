		...
		<div class="col-sm-12 col-md-12">
    		...
    		<nav class="navbar navbar-default" role="navigation">
					<div class="collapse navbar-collapse navbar-ex1-collapse text-right">
				    	<form class="navbar-form navbar-right" role="search" action="datakindinside_list_popup.do" method="post" name="searchForm" id="searchForm">
				    	...
				    	<div class="form-group">
				    		...
				    		<select class="form-control input-sm"   name="search_target" id="search_target" title="검색">
								...
								<option value="dta_knd_inside_kor_nm" <c:if test="${searchInfo.search_target eq 'dta_knd_inside_kor_nm'}">selected="selected"</c:if> >한글명</option>
								<option value="dta_knd_inside_eng_nm" <c:if test="${searchInfo.search_target eq 'dta_knd_inside_eng_nm'}">selected="selected"</c:if> >영문명</option>
							</select>
	        ...
	        <table id="searchResultList" class="table table-striped">
	            ...
	            <tr>
	            	...
	            	<td class="col-sm-2 text-left"><spring:escapeBody>${o.dta_knd_kor_nm}</spring:escapeBody></td>
	            	<td class="col-sm-2 text-left"><a href="javascript:goSelect(${o.dta_knd_inside_seq_n}, '${o.dta_knd_inside_kor_nm}', '${o.dmn_kor_nm}', '${o.dmn_sub_kor_nm}', '${o.dta_knd_kor_nm}' );"><spring:escapeBody>${o.dta_knd_inside_kor_nm}</spring:escapeBody></a></td>
	                <td class="col-sm-2 text-left"><spring:escapeBody>${o.dta_knd_inside_eng_nm}</spring:escapeBody></td>
...
$(function() {
	...
	$("#dataKindSearch, #search_dta_knd_kor_nm").click(function() {
		datakind_win = window.open('datakind_list_popup.do', 'datakind_win','width=1000,height=630,toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, directories=no, status=no');
		datakind_win.focus();
	});	
...
//자료종류 내부 정보 설정
function goSelect(seq, nm, dmnNm, dmnSubNm, dtaKindNm){
	window.opener.setDataKindInside(seq, nm, dmnNm, dmnSubNm, dtaKindNm);
	window.close();
}
