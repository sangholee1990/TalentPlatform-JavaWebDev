<%@ page language="java" contentType="text/html;charset=utf-8" pageEncoding="UTF-8" trimDirectiveWhitespaces="true"
	import="java.util.*,com.gaia3d.web.dto.DomainSubDTO,org.springframework.util.*"
...
<div id="contents" class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10">
			...
			<form:form commandName="domainSubDTO" action="domainsub_submit.do" method="post" class="form-horizontal" role="form">
				<input type="hidden" name="dmn_sub_seq_n" value="${domainSubDTO.dmn_sub_seq_n}"/>
				<input type="hidden" name="mode" value="${mode}"/>
				<input type="hidden" name="dmn_seq_n" id="dmn_seq_n" value="${domainSubDTO.dmn_seq_n}"/>
				...
				<div class="form-group form-group-sm">
			    	<label for="dmn_sub_cd" class="col-sm-2 control-label">코드</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_sub_cd" class="form-control"/>
			    	</div>
			  	</div>
				<div class="form-group form-group-sm">
			    	<label for="dmn_sub_kor_nm" class="col-sm-2 control-label">한글명</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_sub_kor_nm" class="form-control"/>
			    	</div>
			  	</div>
	  	
				<div class="form-group form-group-sm">
			    	<label for="dmn_sub_eng_nm" class="col-sm-2 control-label">영문명</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_sub_eng_nm" class="form-control"/>
			    	</div>
			  	</div>
				...
				<div class="form-group">
			    	<label for="dmn_sub_desc" class="col-sm-2 control-label">설명</label>
			    	<div class="col-sm-5">
			      		<form:textarea path="dmn_sub_desc" class="form-control" rows="5"/>
			      	</div>
			  	</div>	  	
...
$(function() {
	...
	$(paramForm).find('input[type=hidden]').each(function() {
		$("#domainSubDTO").addHidden($(this).attr("id"), $(this).val());
	});
	...
	$("#contents :button").click(function() {
		...
		case "cancelBtn":
			//수정
			<c:if test="${mode eq 'update' }">
				$(paramForm).attr("action", "domainsub_view.do");
			</c:if>

			//등록
			 <c:if test="${mode eq 'new' }">
				$(paramForm).attr("action", "domainsub_list.do");
			</c:if>
