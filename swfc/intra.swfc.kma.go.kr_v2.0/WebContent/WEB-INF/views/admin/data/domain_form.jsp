...
<div id="contents" class="container">
	...
	<div class="row">
		...
		<div class="col-sm-10 col-md-10">
			...
			<form:form commandName="domainDTO" action="domain_submit.do" method="post" class="form-horizontal" role="form">
				<input type="hidden" name="dmn_seq_n" value="${domainDTO.dmn_seq_n}"/>
				<input type="hidden" name="mode" value="${mode}"/>
				<input type="hidden" name="del_f_cd" value="N" />	
				<form:errors path="*" cssClass="errorBlock" element="div"/>

				<div class="form-group form-group-sm">
			    	<label for="dmn_cd" class="col-sm-2 control-label">코드</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_cd" class="form-control"/>
			    	</div>
			  	</div>
	  	
				<div class="form-group form-group-sm">
			    	<label for="dmn_kor_nm" class="col-sm-2 control-label">한글명</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_kor_nm" class="form-control"/>
			    	</div>
			  	</div>
	  	
				<div class="form-group form-group-sm">
			    	<label for="dmn_eng_nm" class="col-sm-2 control-label">영문명</label>
			    	<div class="col-sm-5">
			      		<form:input path="dmn_eng_nm" class="form-control"/>
			    	</div>
			  	</div>
				...
				<div class="form-group">
			    	<label for="dmn_desc" class="col-sm-2 control-label">설명</label>
			    	<div class="col-sm-5">
			      		<form:textarea path="dmn_desc" class="form-control" rows="5"/>
			      	</div>
			  	</div>	  	
...
$(function() {
	...
	$(paramForm).find('input[type=hidden]').each(function() {
		$("#domainDTO").addHidden($(this).attr("id"), $(this).val());
	});
	...
	$("#contents :button").click(function() {
		...
		case "cancelBtn":
			...
			//수정
			<c:if test="${mode eq 'update' }">
				$(paramForm).attr("action", "domain_view.do");
			</c:if>
